...
export default function UserPost({ post, queryKey }: IProps) {
  const [showComments, setShowComments] = useState(true);
  const [commentsLimit, setCommentsLimit] = useState(50);
  const [isEditing, setIsEditing] = useState(false);
  const { userInfo } = useAuth();
  // NOTE: FIX MUTATIONS ON SINGLE POST MUTATIONS ARE NOT UPDATED BECAUSE THE CACHE STRUCTURE IS DIFFERENT FROM INFINITE QUERIES
